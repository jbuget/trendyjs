...
function _updateServicesData(snapshot) {
  return new Promise((resolve, reject) => {
    const promises = [];
    snapshot.get('data').forEach(serviceData => {
      serviceData.date = snapshot.get('created_at');
      promises.push(Service.findOneAndUpdate({ ref: serviceData.references.trendyjs }, { history: [] }).exec());
    });
