...
router.get('/stackoverflow', (req, res, next) => {
  const services = [
    { ref: 'react', stackoverflowTag: 'reactjs' },
    { ref: 'aurelia', stackoverflowTag: 'aurelia' },
    { ref: 'ember', stackoverflowTag: 'ember.js' }
  ];
  stackoverflowRepository.fetchData(services).then(data => {
    res.send(data);
  });
});
